...
Operators should be able to install a new package containing all data
needed to join the resumed network.

The SNARKed ledger can be stored in serialized format, stored as a value
in an generated OCaml module, which can be loaded when creating the
...
the saved breadcrumb into the binary. The breadcrumb data would be
